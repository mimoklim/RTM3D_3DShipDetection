...
class CarPoseLoss(torch.nn.Module):
    ...
    def forward(self, outputs, batch,phase=None):
        ...
        dim_loss, rot_loss, prob_loss, pix_dist_loss = 0, 0, 0, 0
        coor_loss =0
        box_score=0
        output = outputs[0]
        ...
        if opt.pix_dist_weight > 0:
            pix_dist_loss = self.crit_reg(output['pix_dist'], batch['reg_mask'],batch['ind'], batch['pix_dist'])
        ...
        loss_stats = {'loss': box_score, 'hm_loss': hm_loss, 'hp_loss': hp_loss,
                      ...
                      'wh_loss': wh_loss, 'off_loss': off_loss,'dim_loss': dim_loss,'pix_dist_loss': pix_dist_loss,
...
class CarPoseTrainer(BaseTrainer):
    ...
    def _get_losses(self, opt):
        loss_states = ['loss', 'hm_loss', 'hp_loss', 'hm_hp_loss',
                       'hp_offset_loss', 'wh_loss', 'off_loss','dim_loss','pix_dist_loss','rot_loss','prob_loss','coor_loss','box_score']
