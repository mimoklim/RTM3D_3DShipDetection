...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--pix_dist_weight', type=float, default=1,
                             help='loss weight for pixel.')
    ...
    self.parser.add_argument('--eval_oracle_pix_dist', action='store_true', 
                             help='use ground truth pixel.')
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    ...
    opt.heads = {'hm': opt.num_classes, 'wh': 2, 'hps': 18,'rot': 8,'dim':3,'pix_dist':2,'prob':1}
    if opt.reg_offset:
